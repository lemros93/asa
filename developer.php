...
async function loadFeatures(runId) {
    ...
    data.forEach(f => {
        html += `
        <tr data-feat-id="${f.id}" style="border-top:1px solid var(--border)">
...
function applyFiltersToAll() {
    ...
    // 2. Skry features ktoré nemajú žiadny viditeľný scenár
    document.querySelectorAll('tr[data-feat-id]').forEach(featRow => {
        ...
        if (next && next.id && next.id.startsWith('frow-') && !anyVisible) {
            next.style.display = 'none';
        }
    });
}
