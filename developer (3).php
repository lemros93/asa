...
<!-- Scenario search filter + Failed toggle -->
<div style="margin-bottom:12px;display:flex;align-items:center;gap:10px;flex-wrap:wrap">
    ...
    <button class="quick-btn" id="failed-toggle" onclick="toggleFailedOnly()" title="Zobraziť len failed scenáre">
        ❌ Len failed
    </button>
</div>
...
async function loadScenarios(featId) {
    ...
    // Apply existing filters to newly loaded rows
...
function applyFiltersToAll() {
    const hasTextFilter   = !!scenFilterRegex;
    const hasFailedFilter = failedOnly;
    const hasAnyFilter    = hasTextFilter || hasFailedFilter;

    if (!hasAnyFilter) {
        // Zruš všetky filtre — obnov všetky riadky
        document.querySelectorAll('tr[data-run-id], tr[data-feat-id], tr[data-scen-name]').forEach(r => {
            r.style.display = '';
        });
        return;
    }
    ...
    document.querySelectorAll('tr[data-scen-name]').forEach(row => {
        const matchesText   = !hasTextFilter   || scenFilterRegex.test(row.dataset.scenName);
        const matchesFailed = !hasFailedFilter || row.dataset.scenStatus === 'failed';
        const show = matchesText && matchesFailed;
    ...
    // ── 2. Features — skry ak nemajú viditeľný scenár ───────────
    // Ak scenáre ešte nie sú načítané (feature nie je rozbalená),
    // neskrývame feature — necháme ju viditeľnú, urobí si to až pri rozbalení
    document.querySelectorAll('tr[data-feat-id]').forEach(featRow => {
        ...
        if (scenRows.length === 0) return; // nie sú načítané — neskrývaj
    ...
    // ── 3. Runy ─────────────────────────────────────────────────
    // Run je viditeľný ak:
    //   A) jeho run-level polia (build, branch...) matchujú text filter, ALEBO
    //   B) má aspoň jeden viditeľný scenár/feature (text bol nájdený hlbšie)
    // Failed filter sa aplikuje vždy na run úrovni
    document.querySelectorAll('tr[data-run-id]').forEach(runRow => {
        const runId     = runRow.dataset.runId;
        const runSearch = runRow.dataset.runSearch || '';
        const runFailed = runRow.dataset.runFailed === 'yes';

        const failedOK  = !hasFailedFilter || runFailed;

        // Skontroluj či má viditeľné features
        const featRows      = document.querySelectorAll(`tr[data-run-ref="${runId}"]`);
        const hasVisibleFeat = Array.from(featRows).some(r => r.style.display !== 'none');

        // Run matchuje text ak: build/branch matchuje ALEBO má viditeľné features (scenár matchoval)
        const runMatchesText = !hasTextFilter
            || scenFilterRegex.test(runSearch)
            || hasVisibleFeat;

        const show = failedOK && runMatchesText;
...
// Volá sa po načítaní scenárov — preaplikuje filter na nové riadky
function reapplyFiltersAfterLoad() {
    if (scenFilterRegex || failedOnly) applyFiltersToAll();
}
