...
async function loadScenarios(featId) {
    ...
    // Apply existing filters (text + failed-only)
...
function applyFiltersToAll() {
    ...
    // 1. Scenáre — filter podľa textu a failed
    document.querySelectorAll('tr[data-scen-name]').forEach(row => {
        const show = !hasAnyFilter
            || ((!hasTextFilter   || scenFilterRegex.test(row.dataset.scenName))
             && (!hasFailedFilter || row.dataset.scenStatus === 'failed'));
        row.style.display = show ? '' : 'none';
        // Schovaj expand row stepu
    ...
    // 2. Features — skry ak nemajú žiadny viditeľný scenár (keď sú načítané)
    document.querySelectorAll('tr[data-feat-id]').forEach(featRow => {
        ...
        if (!hasAnyFilter) {
            show = true;
        } else if (scenRows.length === 0) {
            // Scenáre ešte nie sú načítané — feature ostane viditeľná
            show = true;
        } else {
            show = Array.from(scenRows).some(r => r.style.display !== 'none');
        }
    ...
    // 3. Runy — filter na run úrovni (build, branch atď.) + skry ak nemajú viditeľné features
    document.querySelectorAll('tr[data-run-id]').forEach(runRow => {
        ...
        if (!hasAnyFilter) {
            ...
            // Text filter na run poli (build, branch, commit...)
            const runMatchesText   = !hasTextFilter   || scenFilterRegex.test(runSearch);
            const runMatchesFailed = !hasFailedFilter || runFailed;

            if (runMatchesText && runMatchesFailed) {
                // Run vyhovuje — skontroluj features ak sú načítané
                const featRows = document.querySelectorAll(`tr[data-run-ref="${runId}"]`);
                if (featRows.length === 0) {
                    // Features ešte nie sú načítané — run je viditeľný
                    show = true;
                } else {
                    // Features sú načítané — ukáž run len ak má viditeľnú feature
                    show = Array.from(featRows).some(r => r.style.display !== 'none');
                }
            } else {
                // Run text/failed filter nevyhovuje — skry
                show = false;
            }
        }

        runRow.style.display = show ? '' : 'none';
        // Expand row runu — skry ak run je skrytý
