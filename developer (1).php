...
function applyFiltersToAll() {
    ...
    // 1. Scenáre — zobraz/skry podľa filtrov
    document.querySelectorAll('tr[data-scen-name]').forEach(row => {
        let show;
        if (!hasFilter) {
            show = true;
        } else {
            const matchesText   = !scenFilterRegex || scenFilterRegex.test(row.dataset.scenName);
            const matchesFailed = !failedOnly || row.dataset.scenStatus === 'failed';
            show = matchesText && matchesFailed;
        }
        row.style.display = show ? '' : 'none';
        // Schovaj expand row stepu ak scenár nie je viditeľný
    ...
    // 2. Features — skry ak nemajú žiadny viditeľný scenár
    document.querySelectorAll('tr[data-feat-id]').forEach(featRow => {
        ...
        if (!hasFilter) {
            ...
            show = scenRows.length === 0 || Array.from(scenRows).some(r => r.style.display !== 'none');
        }
    ...
    // 3. Runy — skry ak nemajú žiadnu viditeľnú feature
    document.querySelectorAll('tr[data-run-id]').forEach(runRow => {
        ...
        if (!hasFilter) {
            ...
            show = featRows.length === 0 || Array.from(featRows).some(r => r.style.display !== 'none');
        }
        runRow.style.display = show ? '' : 'none';
        // Expand row runu schováme len ak run nemá viditeľné features a je rozbalený
        const next = runRow.nextElementSibling;
        if (next && next.id && next.id.startsWith('rrow-') && !show) {
            // Nechaj expand row ako je — len run riadok skryjeme
        }
    });
}
